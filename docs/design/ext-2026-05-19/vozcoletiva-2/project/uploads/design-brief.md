...
- A member can cast one vote per proposal while it's open: **yes**, **no**, or **abstain**.
- The voter can change or retract their vote at any time while voting is open.
- During voting, the running tally is visible.
- When the time is up, the proposal closes automatically (a background process runs at the configured end-time) and the outcome is computed under the rule + quorum. Votes become immutable after close.
